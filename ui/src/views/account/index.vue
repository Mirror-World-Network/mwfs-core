<template xmlns:v-clipboard="http://www.w3.org/1999/xhtml">
  <div>
    ...
    <div
      class="modal"
      id="transfer_accounts_modal"
      v-show="tranferAccountsDialog"
    >
      <div class="modal-dialog">
        <div class="modal-content">
          <div class="modal-header">
    ...
    <div
      class="modal"
      id="batch_transfer_accounts_modal"
      v-show="batchTranferAccountsDialog"
    >
      <div class="modal-dialog">
        <div class="modal-content">
          <div class="modal-header">
    ...
    <div
      class="modal_hubSetting"
      id="hub_setting"
      v-loading="hubsetting.loadingData"
      v-show="hubSettingDialog"
    >
          ...
          <div class="modal-body modal-message">
            <el-form>
              ...
              <el-form-item :label="$t('transfer.amount')">
                ...
                <input
                  class="el-input__inner"
                  v-model="transfer.exchangeNumber"
                  type="number"
                />
                <label class="input_suffix">{{ $global.unit }}</label>
              </el-form-item>
...
export default {
  ...
  data() {
    ...
    return {
      ...
      userConfig: {
        ...
        useNATService:
          this.$store.state.userConfig["sharder.useNATService"] === "true",
        natClientSecretKey: this.$store.state.userConfig[
      ...
      registerSharderSiteUserRules: {
        sharderAccountPhoneOrEmail: [
          { required: true, message: this.$t("rules.mustRequired") },
        ],
        verificationCode: [
          {
            required: true,
            message: this.$t("binding_validation.verification_title"),
          },
        ],
        setSharderPwd: [
          {
        ...
        confirmSharderPwd: [
          {
            ...
            validator: (rule, value, callback) => {
              if (!value && this.registerSharderSiteUser.setSharderPwd) {
                callback(
                  new Error(this.$t("rules.plz_input_sharder_pwd_again"))
                );
              } else if (value !== this.registerSharderSiteUser.setSharderPwd) {
                callback(
                  new Error(this.$t("rules.inconsistent_sharder_password"))
                );
              } else {
                callback();
              }
              ...
              if (value) {
                if (this.hubsetting.modifyMnemonicWord !== SSO.secretPhrase) {
                  callback(
                    new Error(
                      this.$t(
                        "notification.hubsetting_not_matched_mnemonic_word"
                      )
                    )
                  );
                }
        ...
        confirmPwd: [
          {
            ...
            validator: (rule, value, callback) => {
              if (!value && this.hubsetting.newPwd) {
                ...
                callback(
                  new Error(this.$t("rules.inconsistent_admin_password"))
                );
              } else {
                callback();
              }
  ...
  methods: {
    init() {
      ...
      _this.getAccount(_this.accountInfo.accountRS).then((res) => {
        ...
        if (res.pocScore != null) {
          ...
          _this.pocScoreDetail[0].effectiveBalance =
            res.pocScore.effectiveBalance;
          ...
          _this.pocScoreDetail[0].onlineRateScore =
            res.pocScore.onlineRateScore;
          _this.pocScoreDetail[0].performanceScore =
            res.pocScore.performanceScore;
      ...
      _this.$global
        ...
        .then(() => {
          ...
          if (
            publicKey == null &&
            !SSO.downloadingBlockchain &&
            _this.blockchainState.blockchainState == "UP_TO_DATE"
          ) {
            _this.nonePublicKeyHint = false;
          }
        });
      ...
      _this.$global.sendVerifyCode(requestUrl, sharderAccount, function (res) {
        if (res.success) {
          _this.$message.warning(
            _this.$t("mining.binding_validation.verification_tip1") +
              sharderAccount +
              "," +
              _this.$t("mining.binding_validation.verification_tip1")
          );
    ...
    registerSharderSite() {
      ...
      if (sharderAccount === "") {
        ...
        _this.$message.warning(
          _this.$t("mining.binding_validation.verification_title")
        );
        return false;
      } else if (setSharderPwd === "") {
        _this.$message.warning(
          _this.$t("password_modal.input_sharder_site_pwd")
        );
        return false;
    ...
    getLatestHubVersion() {
      const _this = this;
      _this.$http
        .get(_this.$global.urlPrefix() + "?requestType=getLatestCosVersion")
        .then((res) => {
          if (res.data.success) {
            _this.latesetVersion = res.data.cosver.version;
            _this.upgradeMode = res.data.cosver.mode;
            _this.bakMode = res.data.cosver.bakMode;
            let bool = _this.versionCompare(
              _this.blockchainState.version,
              _this.latesetVersion
            );
            _this.isUpdate = bool;
          } else {
            _this.$message.error(
              res.data.error ? res.data.error : res.data.errorDescription
            );
          }
        })
        .catch((err) => {
    ...
    drawBarChart: function (barchat) {
      ...
      if (!dom) {
        console.log(
          "dom transaction_amount_bar got faild，echarts can not load"
        );
        return;
      }
      const _this = this;
    ...
    displaySerialNo: function (clickType) {
      ...
      console.log(
        "clickType=" +
          clickType +
          ",initialSerialClickCount=" +
          _this.hubsetting.initialSerialClickCount +
          ",settingSerialClickCount=" +
          _this.hubsetting.settingSerialClickCount +
          ",natSerialClickCount=" +
          _this.hubsetting.natSerialClickCount
      );
    ...
    updateHubVersion(adminPwd) {
      ...
      this.$http
        .post("/sharder?requestType=upgradeClient", data)
        .then((res) => {
    ...
    restartHub(adminPwd) {
      ...
      data.append("adminPassword", adminPwd);
      this.$http
        .post("/sharder?requestType=restart", data)
        .then((res) => {
    ...
    resetHub(adminPwd, type) {
      ...
      this.$http
        .post("/sharder?requestType=recovery", resetData)
        .then((res) => {
          if (res.data.done) {
            _this.$message.success(_this.$t("restart.restarting"));
            _this.$store.state.mask = false;
    ...
    updateHubSetting(params) {
      const _this = this;
      this.$http
        .post("/sharder?requestType=reConfig", params)
        .then((res) => {
    ...
    verifyAndGenerateHubSettingFormData() {
      ...
      if (_this.hubsetting.openPunchthrough) {
        ...
        if (
          ...
          if (_this.hubsetting.sharderPwd === "")
            _this.$message.error(
              _this.$t("notification.hubsetting_no_sharder_account")
            );
          else
            _this.$message.error(
              _this.$t("notification.hubsetting_sharder_account_no_permission")
            );
          return false;
        } else {
          formData.append(
      ...
      if (_this.hubsetting.SS_Address !== "") {
        if (
          !_this.hubsetting.SS_Address.toUpperCase().match(
            _this.$global.pattern
          )
        ) {
          _this.$message.warning(
            _this.$t("notification.hubsetting_account_address_error_format")
          );
          return false;
        } else {
          formData.append(
      ...
      if (_this.hubsetting.isOpenMining) {
        ...
        if (_this.hubsetting.modifyMnemonicWord === "") {
          _this.$message.warning(
            _this.$t("notification.hubsetting_no_mnemonic_word")
          );
          return false;
        }
        if (SSO.secretPhrase === "") {
          _this.$message.warning(
            _this.$t("notification.hubsetting_login_again")
          );
          return false;
        }
        ...
        if (_this.hubsetting.modifyMnemonicWord !== SSO.secretPhrase) {
          _this.$message.warning(
            _this.$t("notification.hubsetting_not_matched_mnemonic_word")
          );
          return false;
        }
        formData.append(
          "sharder.HubBindPassPhrase",
          _this.hubsetting.modifyMnemonicWord
        );
      } else {
      ...
      if (
        _this.hubsetting.newPwd !== "" ||
        _this.hubsetting.confirmPwd !== ""
      ) {
        if (_this.hubsetting.newPwd !== _this.hubsetting.confirmPwd) {
          _this.$message.warning(
            _this.$t("notification.hubsetting_inconsistent_password")
          );
          return false;
        } else {
    ...
    registerNatService() {
      ...
      this.$http
        .post(getCommonFoundationApiUrl(FoundationApiUrls.natRegister), data)
        .then((response) => {
          _this.registerNatLoading = false;
          if (response.data.success) {
            console.info("success to register NAT service");
            _this.$message.success(
              _this.$t("notification.success_to_register_nat")
            );
            _this.closeDialog();
          } else {
            _this.$message.error(
              `errorCode:${response.data.code}, reason:${response.data.msg}`
            );
            _this.closeDialog();
            this.$refs["reconfigureForm"].clearValidate();
            this.$refs["reconfigureForm"].resetFields();
    ...
    bindNatService() {
      ...
      this.$http
        .post(
          getCommonFoundationApiUrl(FoundationApiUrls.natReservedBinding),
          data
        )
        .then((response) => {
          if (response.data.success) {
            ...
            _this.$message.success(
              _this.$t("notification.success_to_bind_nat")
            );
            _this.closeDialog();
          } else {
            _this.$message.error(
              _this.$t("binding_reservation_penetration_service")
            );
            _this.closeDialog();
            this.$refs["reconfigureForm"].clearValidate();
            this.$refs["reconfigureForm"].resetFields();
    ...
    reconfigure(data) {
      let _this = this;
      this.$http
        .post("/sharder?requestType=reConfig", data)
        .then((res1) => {
          if (res1.data.reconfiged) {
            ...
            let msg = res1.data.errorDescription
              ? res1.data.errorDescription
              : res1.data.failedReason
              ? res1.data.failedReason
              : "error";
    ...
    checkSiteAccount() {
      ...
      if (
        ...
        _this.$http
          .post(
            getCommonFoundationApiUrl(FoundationApiUrls.fetchNatServiceConfig),
            formData
          )
          .then((res) => {
            ...
            if (res.data.success && res.data.data) {
              ...
              _this.hubsetting.register_status_text = this.formatRegisterStatus(
              ...
              _this.$message.error(
                _this.$t(
                  "notification.hubsetting_sharder_account_no_permission"
                )
              );
            } else {
              _this.clearHubSetting();
    ...
    getAccount(account) {
      ...
      return new Promise((resolve, reject) => {
        this.$http
          .get(_this.$global.urlPrefix() + "?requestType=getAccount", {
            params: {
              account: account,
              includeLessors: true,
              includeAssets: true,
    ...
    getMessageFee: function () {
      ...
      if (_this.messageForm.isEncrypted) {
        if (
          ...
          _this.$message.warning(
            _this.$t("notification.sendmessage_null_account")
          );
          return;
        }
        if (_this.messageForm.publicKey === "") {
          _this.$message.warning(
            _this.$t("notification.sendmessage_null_account_public")
          );
          return;
        }
        if (!(_this.messageForm.password || _this.secretPhrase)) {
          _this.$message.warning(
            _this.$t("notification.sendmessage_null_secret_key")
          );
          return;
        }
        ...
        if (_this.messageForm.isFile) {
          ...
          let encryptionkeys = SSO.getEncryptionKeys(
            options,
          ...
          _this.encryptFileCallback(_this.file, encryptionkeys).then((res) => {
            ...
            formData.append(
              ...
              converters.byteArrayToHexString(res.nonce)
            );
            _this.sendMessage(formData);
          });
        } else {
          encrypted = SSO.encryptNote(
        ...
        if (_this.messageForm.isFile) {
          ...
          if (_this.messageForm.message !== "") {
            ...
            if (
              _this.$global.stringToByte(_this.messageForm.message).length >= 28
            ) {
              //28 MIN_PRUNABLE_MESSAGE_LENGTH
              formData.append("messageIsPrunable", "true");
            }
          }
        }
    ...
    getTransferFee: function () {
      ...
      if (!_this.transfer.receiver.toUpperCase().match(_this.$global.pattern)) {
        _this.$message.warning(
          _this.$t("notification.sendmessage_account_error_format")
        );
        return;
      }
      ...
      _this.getAccount(_this.accountInfo.accountRS).then((res) => {
        if (typeof res.errorDescription === "undefined") {
          if (res.errorDescription === "Unknown account") {
            _this.$message.warning(
              _this.$t("notification.new_account_warning")
            );
            return;
          }
        }
        ...
        if (
          _this.transfer.number >
          _this.accountInfo.effectiveBalanceNQT / _this.$global.unitValue
        ) {
          _this.$message.warning(
            _this.$t("notification.transfer_balance_insufficient")
          );
          return;
        }
        if (!(_this.secretPhrase || _this.transfer.password)) {
          _this.$message.warning(
            _this.$t("notification.transfer_null_secret_key")
          );
          return;
        }
        ...
        formData.append(
          "amountNQT",
          new BigNumber(_this.transfer.number).times(_this.$global.unitValue)
        );
        ...
        if (_this.transfer.hasMessage && _this.transfer.message !== "") {
          if (_this.transfer.isEncrypted) {
            ...
            if (_this.transfer.publicKey === "") {
              _this.$message.warning(
                _this.$t("notification.transfer_null_public_key")
              );
              return;
            }
            ...
            encrypted = SSO.encryptNote(
              ...
              options,
              _this.secretPhrase || _this.transfer.password
            );
            formData.append("encrypt_message", "1");
            formData.append("encryptedMessageData", encrypted.message);
            formData.append("encryptedMessageNonce", encrypted.nonce);
    ...
    uploadFile: function () {
      ...
      formData.append(
        "feeNQT",
        new BigNumber(_this.messageForm.fee).times(_this.$global.unitValue)
      );
      formData.append(
        "secretPhrase",
        _this.messageForm.password || _this.secretPhrase
      ...
      _this.$http
        .post(
          _this.$global.urlPrefix() + "?requestType=storeData",
          formData,
          config
        )
        .then((res) => {
    ...
    onChain: function () {
      ...
      formData.append(
        "feeNQT",
        new BigNumber(_this.messageForm.fee).items(_this.$global.unitValue)
      );
      formData.append(
        "secretPhrase",
        _this.messageForm.password || _this.secretPhrase
      ...
      _this.$http
        .post("/sharder?requestType=onChain", formData, config)
        .then((res) => {
          if (typeof res.data.errorDescription === "undefined") {
            if (res.data.broadcasted) {
              _this.$message.success(_this.$t("notification.upload_success"));
    ...
    sendMessageInfo: function () {
      ...
      if (
        !_this.messageForm.receiver.toUpperCase().match(_this.$global.pattern)
      ) {
        _this.$message.warning(
          _this.$t("notification.sendmessage_account_error_format")
        );
      ...
      SSO.sendMessage(formData, function (res) {
        ...
        if (typeof res.errorDescription === "undefined") {
          if (res.broadcasted) {
            ...
            _this.closeDialog();
            _this.$global
              .setUnconfirmedTransactions(_this, SSO.account)
              .then((res) => {
            ...
            console.log(res);
            _this.transfer.fee =
              res.transactionJSON.feeNQT / _this.$global.unitValue;
          }
        } else {
          if (res.errorDescription.indexOf("$.t") != -1) {
            _this.$message.error(
              _this.$global.escape2Html(_this.$t(res.errorDescription.slice(3)))
            );
          } else {
            _this.$message.error(
              _this.$global.escape2Html(res.errorDescription)
            );
          }
        }
        _this.messageForm.executing = false;
    ...
    sendBatchMoney(formData) {
      ...
      return new Promise((resolve, reject) => {
        this.$http
          .post("/sharder?requestType=airdrop", formData, config)
          .then(function (res) {
            resolve(res.data);
          })
          .catch(function (err) {
    ...
    detectionBatchMoney(formData) {
      ...
      return new Promise((resolve, reject) => {
        this.$http
          .post("/sharder?requestType=airdropDetection", formData, config)
          .then(function (res) {
            resolve(res.data);
          })
          .catch(function (err) {
    ...
    sendBatchTransferInfo: function () {
      ...
      if (!secretPhrase) {
        _this.$message.warning(
          _this.$t("transfer.airdrop_secret_key_not_found")
        );
        return;
      }
      ...
      _this.sendBatchMoney(formData).then((res) => {
        if (typeof res.errorDescription === "undefined") {
          ...
          _this.$global.funDownload(
            JSON.stringify(res.jsonResult),
    ...
    detectionBatchTransferInfo: function () {
      ...
      _this.detectionBatchMoney(formData).then((res) => {
        if (typeof res.errorDescription === "undefined") {
          ...
          _this.$message.success(
            _this.$t("transfer.detection_transfer_success")
          );
          _this.closeDialog();
        } else {
          _this.$message.error(res.errorDescription);
    ...
    sendTransferInfo: function () {
      ...
      if (
        _this.transfer.receiver === _this.acrossChains.Heco.CosExchangeAddress
      ) {
      ...
      if (
        _this.transfer.receiver === _this.acrossChains.OKEx.CosExchangeAddress
      ) {
      ...
      if (
        _this.transfer.receiver === _this.acrossChains.ETH.CosExchangeAddress
      ) {
      ...
      if (
        _this.transfer.receiver === _this.acrossChains.Tron.CosExchangeAddress
      ) {
      ...
      if (
        _this.transfer.receiver === _this.acrossChains.BSC.CosExchangeAddress
      ) {
      ...
      if (
        parseFloat(_this.transfer.number, 10) === 0 ||
        _this.transfer.number === ""
      ) {
        _this.$message.warning(_this.$t("notification.transfer_amount_error"));
        _this.transfer.executing = false;
      ...
      _this.getAccount(_this.accountInfo.accountRS).then((res) => {
        ...
        if (
          _this.transfer.number.toString().split(".")[1] !== undefined &&
          _this.transfer.number.toString().split(".")[1].length >
            _this.$global.unitValue.toString().length - 1
        ) {
          _this.$message.warning(
            _this.$t("notification.transfer_balance_decimal_not_support")
          );
          _this.transfer.executing = false;
          return;
        }
        ...
        formData.append(
          "feeNQT",
          new BigNumber(_this.transfer.fee).times(_this.$global.unitValue)
        );
        formData.append(
          "amountNQT",
          new BigNumber(_this.transfer.number).times(_this.$global.unitValue)
    ...
    sendExchangeTransferInfo: function () {
      ...
      if (
        ChainObj.CosExchangeAddress === this.$global.receiverPrefixStr ||
      ...
      if (
        !ChainObj.CosExchangeAddress.toUpperCase().match(_this.$global.pattern)
      ) {
        _this.$message.warning(
          _this.$t("notification.sendmessage_account_error_format")
        );
      ...
      _this.getAccount(_this.accountInfo.accountRS).then((res) => {
        ...
        if (
          _this.transfer.exchangeNumber.toString().split(".")[1] !==
            undefined &&
          _this.transfer.exchangeNumber.toString().split(".")[1].length >
            _this.$global.unitValue.toString().length - 1
        ) {
          _this.$message.warning(
            _this.$t("notification.transfer_balance_decimal_not_support")
        ...
        if (
          _this.transfer.exchangeNumber >
          ChainObj.convertible_balance * ChainObj.CosExchangeRate
        ) {
          _this.$message.warning(
            _this.$t("acrossChains.convertible_balance_not_enough")
          );
          _this.transfer.executing = false;
          return;
        }
        ...
        formData.append(
          "recipientPublicKey",
          ChainObj.CosExchangeAddressPublicKey
        );
        formData.append("deadline", "1440");
        formData.append("phased", "false");
        formData.append("phasingLinkedFullHash", "");
        ...
        formData.append(
          "amountNQT",
          _this.transfer.exchangeNumber * _this.$global.unitValue
        );
        formData.append(
          "secretPhrase",
          _this.secretPhrase || _this.transfer.password
    ...
    setMaxConvertibleBalance: function () {
      ...
      this.transfer.exchangeNumber =
        ChainObj.convertible_balance * ChainObj.CosExchangeRate <
        _this.acrossChains.balance
          ? ChainObj.convertible_balance * ChainObj.CosExchangeRate
          : _this.acrossChains.balance;
    },
      ...
      SSO.sendMoney(formData, function (res) {
        if (formData.recipient === _this.acrossChains.Heco.CosExchangeAddress) {
          _this.acrossChains.Heco.convertible_balance -=
            _this.transfer.exchangeNumber /
            _this.acrossChains.Heco.CosExchangeRate;
          ...
          _this.acrossChains.OKEx.convertible_balance -=
            _this.transfer.exchangeNumber /
            _this.acrossChains.OKEx.CosExchangeRate;
          _this.acrossChains.OKEx.target_balance +=
            _this.transfer.exchangeNumber *
            _this.acrossChains.OKEx.CosExchangeRate;
        } else if (_this.acrossChains.ETH.CosExchangeRate) {
          _this.acrossChains.ETH.convertible_balance -=
            _this.transfer.exchangeNumber /
            _this.acrossChains.ETH.CosExchangeRate;
          _this.acrossChains.ETH.target_balance +=
            _this.transfer.exchangeNumber *
            _this.acrossChains.ETH.CosExchangeRate;
        } else if (_this.acrossChains.Tron.CosExchangeRate) {
          _this.acrossChains.Tron.convertible_balance -=
            _this.transfer.exchangeNumber /
            _this.acrossChains.Tron.CosExchangeRate;
          _this.acrossChains.Tron.target_balance +=
            _this.transfer.exchangeNumber *
            _this.acrossChains.Tron.CosExchangeRate;
        } else if (_this.acrossChains.BSC.CosExchangeRate) {
          _this.acrossChains.BSC.convertible_balance -=
            _this.transfer.exchangeNumber /
            _this.acrossChains.BSC.CosExchangeRate;
          _this.acrossChains.BSC.target_balance +=
            _this.transfer.exchangeNumber *
            _this.acrossChains.BSC.CosExchangeRate;
        }
      ...
      _this.$http.get(window.api.getAddress).then(function (res1) {
        ...
        if (result) {
          _this.acrossChains.Heco.CosExchangeAddress =
            result.Heco.CosExchangeAddress;
          _this.acrossChains.Heco.CosExchangeAddressPublicKey =
          ...
          _this.acrossChains.OKEx.CosExchangeAddressPublicKey =
          ...
          _this.acrossChains.ETH.CosExchangeAddressPublicKey =
          ...
          _this.acrossChains.Tron.CosExchangeAddressPublicKey =
          ...
          _this.acrossChains.BSC.CosExchangeAddressPublicKey =
    ...
    openAssetsAcrossChainsDialog: function () {
      ...
      var str = _this.$global.formatNQTMoney(
        _this.accountInfo.effectiveBalanceNQT,
        2
      );
      ...
      if (_this.accountInfo.accountRS) {
        _this.$http
          .get(window.api.getAccountInfoUrl, {
            params: { accountRS: _this.accountInfo.accountRS },
          })
          .then(function (res1) {
            switch (res1.data.code) {
              case "200":
                ...
                _this.acrossChains.Heco.target_balance =
                  account.HecoBalance / _this.$global.HecoUnitValue;
                _this.acrossChains.Heco.convertible_balance =
              ...
              case "209":
                _this.$message.warning(
                  _this.$t("acrossChains.bindAddress_incomplete")
                );
                break;
              case "363":
                _this.$message.warning(
                  _this.$t("acrossChains.bindAddress_double")
                );
                break;
              default:
                break;
    ...
    formatDiskCapacity: function () {
      return (
        parseFloat(this.userConfig.diskCapacity / 1024 / 1024).toFixed(2) +
        " GB"
      );
    },
    ...
    openSecretPhraseDialog: function () {
      ...
      _this.secretPhrase
        ? _this.setName(_this.secretPhrase)
    ...
    getAdminPassword: function (adminPwd) {
      ...
      if (
        _this.adminPasswordTitle === "reset" ||
    ...
    parseFileChange: function (e) {
      ...
      _this.$global
        .readFile(_this.parsefile)
        .then((res) => (_this.batch_transfer.jsonData = res));
        ...
        .then((res) => {
          if (typeof res.data.errorDescription === "undefined") {
            ...
            info.forEach(function (value, index, array) {
              if (value.type === 0) {
                yields.xAxis.push(
                  _this.$global.myFormatTime(value.timestamp, "YMD", true)
                );
                if (value.senderRS !== SSO.accountRS) {
                  ...
                  assets =
                    assets -
                    value.amountNQT / _this.$global.unitValue -
                    value.feeNQT / _this.$global.unitValue;
                }
    ...
    getTotalList: function () {
      ...
      for (let i = 0; i < _this.unconfirmedTransactionsList.length; i++) {
        if (_this.selectType === "") {
          ...
          if (
            _this.selectType === 1 &&
            _this.unconfirmedTransactionsList[i].subtype === 0
          ) {
            list.push(_this.unconfirmedTransactionsList[i]);
            _this.totalSize++;
          } else if (
            _this.selectType !== 1 &&
            _this.selectType === _this.unconfirmedTransactionsList[i].type
          ) {
            list.push(_this.unconfirmedTransactionsList[i]);
            _this.totalSize++;
          } else if (
            _this.selectType === 1.5 &&
    ...
    getAccountRsBySecret() {
      let publicKey = global.SSO.getPublicKey(
        this.hubsetting.modifyMnemonicWord,
        false
      );
    ...
    validationReceiver(val) {
      ...
      if (
        receiver === this.$global.receiverEmptyStr ||
        receiver === this.$global.receiverPrefixStr
      ) {
        return;
      }
    ...
    bindChainAddress() {
      ...
      switch (this.chainId) {
        case 1:
          if (
            this.acrossChains.Heco.target_address ==
            this.acrossChains.Heco.old_address
          ) {
        ...
        case 2:
          if (
            this.acrossChains.OKEx.target_address ==
            this.acrossChains.OKEx.old_address
          ) {
            sameAddressFlag = true;
          }
          break;
        case 3:
          if (
            this.acrossChains.ETH.target_address ==
            this.acrossChains.ETH.old_address
          ) {
            sameAddressFlag = true;
          }
          break;
        case 4:
          if (
            this.acrossChains.Tron.target_address ==
            this.acrossChains.Tron.old_address
          ) {
            sameAddressFlag = true;
          }
          break;
        case 5:
          if (
            this.acrossChains.BSC.target_address ==
            this.acrossChains.BSC.old_address
          ) {
            sameAddressFlag = true;
          }
          break;
      ...
      this.$http
        .post(window.api.updateChainAccountUrl, formData)
        .then(function (res1) {
          switch (res1.data.code) {
            case "200":
              switch (_this.chainId) {
                case 1:
                  _this.acrossChains.Heco.old_address =
                    _this.acrossChains.Heco.target_address;
                  _this.$message.success(
                    _this.$t("acrossChains.bindAddress_success_Heco")
                  );
                  break;
                case 2:
                  _this.acrossChains.OKEx.old_address =
                    _this.acrossChains.OKEx.target_address;
                  _this.$message.success(
                    _this.$t("acrossChains.bindAddress_success_OKEx")
                  );
                  break;
                case 3:
                  _this.acrossChains.ETH.old_address =
                    _this.acrossChains.ETH.target_address;
                  _this.$message.success(
                    _this.$t("acrossChains.bindAddress_success_ETH")
                  );
                  break;
                case 4:
                  _this.acrossChains.Tron.old_address =
                    _this.acrossChains.Tron.target_address;
                  _this.$message.success(
                    _this.$t("acrossChains.bindAddress_success_Tron")
                  );
                  break;
                case 5:
                  _this.acrossChains.BSC.old_address =
                    _this.acrossChains.BSC.target_address;
                  _this.$message.success(
                    _this.$t("acrossChains.bindAddress_success_BSC")
                  );
                  break;
                default:
                  break;
        ...
        .catch((err) => {
          switch (_this.chainId) {
            case 1:
              _this.$message.warning(
                _this.$t("acrossChains.bindAddress_fail_Heco")
              );
              break;
            case 2:
              _this.$message.warning(
                _this.$t("acrossChains.bindAddress_fail_OKEx")
              );
              break;
            case 3:
              _this.$message.warning(
                _this.$t("acrossChains.bindAddress_fail_ETH")
              );
              break;
            case 4:
              _this.$message.warning(
                _this.$t("acrossChains.bindAddress_fail_Tron")
              );
              break;
            case 5:
              _this.$message.warning(
                _this.$t("acrossChains.bindAddress_fail_BSC")
              );
              break;
            default:
              break;
  ...
  computed: {
    ...
    openAirdrop: function () {
      const _this = this;
      if (
        _this.hubsetting.airdropStatus &&
        !_this.$global.isOpenApiProxy() &&
        !_this.airdropFlag
      ) {
  ...
  mounted() {
    ...
    let periodicTransactions = setInterval(
      ...
      SSO.downloadingBlockchain
        ? this.$global.cfg.soonInterval
        : this.$global.isOpenApiProxy()
        ? this.$global.cfg.slowInterval
        : this.$global.cfg.defaultInterval
    );
