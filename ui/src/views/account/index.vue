  ...
  <div>
      ...
      <div class="block_list">
        ...
        <div class="transaction_type">
          <span
            ...
            @click="selectType = 0"
          >{{ $t("transaction.transaction_type_payment") }}</span>
          <span
            class="btn"
            :class="activeSelectType(8)"
            @click="selectType = 8"
          >{{ $t("transaction.transaction_type_forge_pool") }}</span>
          <span
            class="btn"
            :class="activeSelectType(9)"
            @click="selectType = 9"
          >{{ $t("transaction.transaction_type_system_reward") }}</span>
                  ...
                  <td class="image_text w300 pc-table">
                    ...
                    <span v-else-if="transaction.type === 18">
                      <span
                        class="linker"
                        @click="openAccountInfoDialog(transaction.recipientRS)"
                      >{{ $t("transaction.transaction_burn_account") }}</span>
                    </span>
                    <span
                      class="linker"
                      v-else-if="
      ...
      <div class="modal-dialog">
        <div class="modal-content">
          ...
          <div class="modal-body modal-message">
            <el-form>
              ...
              <el-form-item :label="$t('sendMessage.infomation')">
                <el-checkbox
                  v-model="messageForm.isEncrypted"
                >{{ $t("sendMessage.encrypted_information") }}</el-checkbox>
                <el-input
              ...
              <el-form-item :label="$t('sendMessage.fee')">
                <el-button
                  class="calculate_fee"
                  @click="getMessageFee()"
                >{{ $t("sendMessage.calc_short") }}</el-button>
          ...
          <div class="modal-footer">
            ...
            >{{ $t("sendMessage.send_message") }}</button>
          </div>
        </div>
      </div>
    </div>
        ...
        <div class="modal-content">
            ...
            <el-form>
              <el-form-item :label="$t('sendMessage.file')">
                ...
                >
                  ...
                  >{{ $t("sendMessage.browse") }}</el-button>
                  <el-button
                    slot="append"
                    @click="delStorageFile"
                    v-else
                  >{{ $t("sendMessage.delete") }}</el-button>
                </el-input>
                <input
                  id="storageFile"
                  ref="storageFile"
          ...
          <div class="modal-footer">
            <button
              ...
              @click="uploadFile"
              :disabled="isDisable"
            >{{ $t("sendMessage.upload_file") }}</button>
          </div>
        </div>
      </div>
    </div>
      ...
      <div class="modal-dialog">
        <div class="modal-content">
          ...
          <div class="modal-body modal-message">
            <el-form>
              ...
              <el-form-item :label="$t('transfer.fee')">
                <el-button
                  class="calculate_fee"
                  @click="getTransferFee()"
                >{{ $t("transfer.calc_short") }}</el-button>
              ...
              <el-form-item label>
                ...
                >{{ $t("transfer.encrypted_information") }}</el-checkbox>
                <el-input
                  type="textarea"
                  :autosize="{ minRows: 2, maxRows: 10 }"
                  resize="none"
          ...
          <div class="modal-footer">
            ...
            >{{ $t("transfer.transfer_send") }}</button>
          </div>
        </div>
      </div>
    </div>
      ...
      <div class="modal-dialog">
        <div class="modal-content">
            ...
            <el-form>
              <el-form-item :label="$t('sendMessage.json_file')">
                <el-input
                  ...
                  :readonly="true"
                >
                  <el-button slot="append" v-if="parsefile === null">{{ $t("sendMessage.browse") }}</el-button>
                  <el-button
                    slot="append"
                    @click="delParseFile"
                    v-else
          ...
          <div class="modal-footer">
            ...
            >{{ $t("transfer.batch_transfer_send") }}</button>
            <el-row>
              <el-col :span="24">
            ...
            >{{ $t("transfer.batch_transfer_detection") }}</button>
          </div>
        </div>
      </div>
    </div>
    ...
    <div class="modal_hubSetting" id="hub_init_setting" v-show="hubInitDialog">
      ...
      <div class="modal-body">
        ...
        <div class="footer-btn">
          ...
          >{{ $t("hubsetting.confirm_restart") }}</button>
      ...
      <div class="modal-body">
        ...
        <div class="footer-btn">
          <el-button
            ...
            "
          >{{ $t("hubsetting.confirm_restart") }}</el-button>
          <el-button
            class="common_btn imgBtn"
            @click="registerNatService()"
            v-if="hubsetting.openPunchthrough && this.needRegister"
          >{{ $t("hubsetting.register_nat_server") }}</el-button>
          <button class="common_btn writeBtn" @click="closeDialog">{{ $t("hubsetting.cancel") }}</button>
        </div>
      </div>
    </div>
    ...
    <div class="modal_info" id="account_info" v-show="userInfoDialog">
            ...
            <tr>
              ...
              <td>
                ...
                <div class="rewriteName" v-else>
                  ...
                  <button
                    class="common_btn"
                    @click="openSecretPhraseDialog"
                  >{{ $t("account_info.account_set_name") }}</button>
                </div>
              </td>
            </tr>
            <tr>
      ...
      <div class="pocScore-detail">
        <el-table :data="pocScoreDetail" style="width: 100%" :show-header="false">
        ...
        <div class="modal-content">
          ...
          <div class="modal-body modal-message">
            <ul class="title" v-if="!showChain">
              ...
              <li :class="chainId === 7 ? 'active' : ''" @click="showChains(7)">
                <a>...</a>
              </li>
            </ul>
            <ul class="title" v-else>
              ...
              <li :class="chainId === 6 ? 'active' : ''">
                <a>Polygon</a>
              </li>
              <li :class="chainId === 7 ? 'active' : ''">
                <a>...</a>
              </li>
            </ul>
            ...
            <div id="content">
              ...
              <el-form class="mod" v-if="chainId === 6">
                <el-form-item
                  :label="$t('acrossChains.Polygon_target_address')"
                  class="item_address"
                >
                  <el-input
                    id="acrossChains_target_address"
                    v-model="acrossChains.Polygon.target_address"
                    :placeholder="$t('acrossChains.Polygon_address_tip')"
                  >
                    <el-button
                      slot="append"
                      style="background: #3fb09a; color: #fff"
                ...
                <el-form-item
                  :label="$t('acrossChains.Polygon_rate')"
                  class="rate"
                  style="margin-bottom: 0"
                ></el-form-item>
                <div style="width: 100%; height: 40px">
                  <input
                    class="input__mw"
                    v-model="acrossChains.Polygon.rateMW"
                    type="text"
                    disabled="disabled"
                  />
                  <label class="input_unit">{{ $global.unit }}</label>
                  ...
                  <input
                    class="input__heco"
                    v-model="acrossChains.Polygon.ratePolygon"
                    type="text"
                    disabled="disabled"
                  />
                  <label class="input_hecoUnit">{{ $global.PolygonUnit }}</label>
                  <el-button
                    class="input_exchange"
                    slot="append"
                    @click="openExchangeDialog"
                    :disabled="showChain"
                    style="background: #3fb09a; color: #fff"
                  >{{ $t("acrossChains.exchange") }}</el-button>
                </div>
              </el-form>
              <el-form class="mod" v-if="chainId === 7">{{ $t("acrossChains.more") }}</el-form>
            </div>
          </div>
          <div class="modal-footer">
                    ...
                    : chainId === 3
                      ...
                      : chainId === 4
                        ...
                        : chainId === 5
                          ...
                          : chainId === 6
                            ? acrossChains.Polygon.CosExchangeAddress
                            : ""
              }}
            </a>
            {{
                    ...
                    : chainId === 3
                      ...
                      : chainId === 4
                        ...
                        : chainId === 5
                          ...
                          : chainId === 6
                            ? acrossChains.Polygon.contractAddress
                            : ""
              }}
            </a>
          </div>
          ...
          <div class="modal-body modal-message">
            <el-form>
              ...
              >
                <masked-input
                  mask="AAA-****-****-****-*****"
                  v-model="acrossChains.Polygon.CosExchangeAddress"
                  disabled
                />
                <img src="../../assets/img/account_directory.svg" />
              </el-form-item>
              ...
              <el-form-item :label="$t('transfer.amount')">
                ...
                >{{ $t("acrossChains.all_convertible_balance") }}</el-button>
                <el-button
                  class="calculate_fee"
                  @click="setMediumConvertibleBalance()"
                >{{ $t("acrossChains.medium_convertible_balance") }}</el-button> -->
                <input class="el-input__inner" v-model="transfer.exchangeNumber" type="number" disabled/>
                <label class="input_suffix">{{ $global.unit }}</label>
              </el-form-item>
              <el-form-item :label="$t('transfer.fee')">
                <input class="el-input__inner" v-model="transfer.fee" type="number" />
    ...
    <div class="modal" id="showChain" v-show="showChain">
      <div class="modal-dialog">
                    ...
                    : chainId === 3
                      ...
                      : chainId === 4
                        ...
                        : chainId === 5
                          ...
                          : chainId === 6
                            ? acrossChains.Polygon.target_address
                            : ""
              }}
            </a>
          </div>
        ...
        <div class="modal-footer">
          ...
          >{{ $t("acrossChains.sure") }}</el-button>
          <el-button
            class="cancelButton"
            slot="append"
            style="color: #000"
            @click="cancel()"
          >{{ $t("acrossChains.cancel") }}</el-button>
        </div>
      </div>
    </div>
...
export default {
  ...
  data() {
    ...
    return {
      ...
      exchangeTransactionType: [
        ...
        {
          value: 6,
          label: this.$t("transaction.transaction_type_mw_to_polygon"),
        },
        // {
        //   value: 7,
      ...
      acrossChains: {
        ...
        Polygon: {
          ...
          rateMW: 10,
          ratePolygon: 1,
          CosExchangeAddress: "CDW-XXXX-XXXX-XXXX-Polygon",
          CosExchangeAddressPublicKey: "",
          CosExchangeRate: 1,
          ExchangeAddress: "0x0Polygon",
          contractAddress: "0x0000Polygon",
        },
        balance: 0,
        id: ''
      },
  ...
  methods: {
    ...
    getAccountExchangeTransactionList: function () {
      ...
      if (_this.exchangeSelectType === 6) {
        params.append(
          "recipientRS",
          _this.acrossChains.Polygon.CosExchangeAddress
        );
      }
    ...
    getAddress: function () {
      ...
      _this.$http.get(window.api.getAddress).then(function (res1) {
        ...
        if (result) {
          ...
          _this.acrossChains.Polygon.CosExchangeAddress = result.Polygon.CosExchangeAddress;
          _this.acrossChains.Polygon.CosExchangeAddressPublicKey = result.Polygon.CosExchangeAddressPublicKey;
          _this.acrossChains.Polygon.CosExchangeRate = result.Polygon.CosExchangeRate;
          _this.acrossChains.Polygon.ExchangeAddress = result.Polygon.ExchangeAddress;
          _this.acrossChains.Polygon.contractAddress = result.Polygon.contractAddress;

           _this.acrossChains.BlackList = result.BlackList;
    ...
    openAssetsAcrossChainsDialog: function () {
      ...
      _this.acrossChains.BlackList.forEach(function(blackAccont){
        if(_this.accountInfo.accountId == blackAccont){
          this.$message.warning(this.$t("acrossChains.inBlackList"));
          return;
        }
      })
        ...
        _this.$http.get(window.api.getAccountInfoUrl, { params: { accountRS: _this.accountInfo.accountRS } }).then(function (res1) {
          switch (res1.data.code) {
            case "200":
              ...
              _this.acrossChains.Polygon.target_address = account.polygonAddress;
    ...
    openExchangeDialog: function () {
      ...
      switch (this.chainId) {
        ...
        //   break;
        case 6:
          if (this.acrossChains.Polygon.target_address === "") {
            return this.$message.warning(this.$t("acrossChains.no_PolygonAddress"));
          }
          break;
        default:
          break;
      ...
      var moneyValue = this.$global.formatNQTMoney(this.accountInfo.effectiveBalanceNQT);
      var money = moneyValue.substring(0,moneyValue.length-2);
      console.log('moneyValue',moneyValue.substring(0,moneyValue.length-2))
      this.transfer.exchangeNumber = parseFloat(money)-1;
      this.AssetsAcrossChainsDialog = false;
      this.transfer.executing = false;
    },
    ...
    bindChainAddress() {
      ...
      switch (this.chainId) {
        ...
        //   break;
        case 6:
          if (this.acrossChains.Polygon.target_address == this.acrossChains.Polygon.old_address) {
            sameAddressFlag = true;
          }
          break;
        default:
      ...
      if (
        ...
        // _this.acrossChains.BSC.target_address ==
        // _this.acrossChains.BSC.CosExchangeAddress
        _this.acrossChains.Polygon.target_address ==
        _this.acrossChains.Polygon.CosExchangeAddress
      ) {
          ...
          switch (res1.data.code) {
            case "200":
              switch (_this.chainId) {
                ...
                //   break;
                case 6:
                  _this.acrossChains.Polygon.old_address = _this.acrossChains.Polygon.target_address;
                  _this.$message.success(_this.$t('acrossChains.bindAddress_success_Polygon'));
                  break;
                default:
                  break;
              }
        ...
        .catch(err => {
          switch (_this.chainId) {
            ...
            case 5:
              ...
              break;
            case 6:
              _this.$message.warning(_this.$t('acrossChains.bindAddress_fail_Polygon'));
              break;
            default:
              break;
          }
    ...
    sendExchangeTransferInfo: function () {
      ...
      switch (_this.chainId) {
        ...
        //   break;
        case 6:
          ChainObj = _this.acrossChains.Polygon;
          break;
        default:
          break;
      }
    ...
    setMaxConvertibleBalance: function () {
      ...
      //     break;
      // }
      _this.transfer.exchangeNumber = _this.accountInfo.effectiveBalanceNQT ? _this.accountInfo.effectiveBalanceNQT / _this.$global.unitValue : 0;
    },
    ...
    setMediumConvertibleBalance: function () {
      ...
      //     break;
      // }
      _this.transfer.exchangeNumber = _this.accountInfo.effectiveBalanceNQT ? (_this.accountInfo.effectiveBalanceNQT /_this.$global.unitValue) / 2 : 0;
    },
